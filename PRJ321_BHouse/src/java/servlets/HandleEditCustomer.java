 ...
 * @author lenovo
 */
public class HandleEditCustomer extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int numberRoomOld = Integer.parseInt(session.getAttribute("oldRoomNumber").toString());
            ...
            if (checkUpdate == true) {
                ...
                try {
                    if (dbContext.getAvailableRoom(numberRoom) == 1) {
                        dbContext.updateCustomer(ID, name, card, phoneNumber, parentsPhoneNumber, roomNumber, dateJoin);
                        dbContext.changeNumpersonRoom(numberRoom, 1);
                        dbContext.changeNumpersonRoom(numberRoomOld, -1);
                        if (dbContext.getNumPersoninRoom(numberRoom) == 3) {
                            dbContext.changeAvailble(numberRoom, 0);
                        } else if(dbContext.getNumPersoninRoom(numberRoomOld) < 3){
                            dbContext.changeAvailble(numberRoomOld, 1);
                        }
                    ...
                    session.setAttribute("date", "Error input Date" + "<br>");
                    response.sendRedirect("editCustomer.jsp?customerID=" + customerID);
                }
